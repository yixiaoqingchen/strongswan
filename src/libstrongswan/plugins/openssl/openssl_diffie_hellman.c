...
static inline void DH_get0_key(DH *dh, const BIGNUM **pk, const BIGNUM **privk)
{
	if (pk)
	{
		*pk = dh->pub_key;
...
#define DH_set0_key(dh, pubkey, privkey) ({ (dh)->priv_key = privkey; TRUE; })
#define DH_set0_pqg(dh, new_p, q, new_g) ({ (dh)->p = new_p; (dh)->g = new_g; TRUE; })
#define DH_set_length(dh, len) ({ (dh)->length = len; TRUE; })
#endif
